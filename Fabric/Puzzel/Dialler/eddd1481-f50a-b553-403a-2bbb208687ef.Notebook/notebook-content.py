...
spark.sql("SHOW TABLES IN Golden_lakehouse.dbo").show(truncate=False)
...
WITH ranked AS (
    ...
    FROM Golden_lakehouse.dbo.ProductHistory AS ph
    INNER JOIN Golden_lakehouse.dbo.Product_config AS PC
        ON PC.id = ph.ProduktID
    INNER JOIN Golden_lakehouse.dbo.Customer AS C
        ON ph.accountKey = C.accounts_id
    INNER JOIN Golden_lakehouse.dbo.Cust_Group AS GC
...
SELECT
    KundeNO
FROM Golden_lakehouse.dbo.ProductHistory
INNER JOIN Golden_lakehouse.dbo.Product_config AS PC ON PC.id = ProductHistory.ProduktID
...
bronze_df = spark.table("Bronze_lakehouse.dbo.all_data_customer_table") \
