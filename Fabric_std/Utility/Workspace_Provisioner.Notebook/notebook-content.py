...
# | Workspace | Contents |
# |---|---|
# | Raw Landing Zone | Mirrored database (GenericMirror / Open Mirroring — source connection configured separately) |
# | Dev / Test / Prod - Dataplatform | Folders: Raw, Enriched, Curated — each with a matching lakehouse |
...
# 
# **Auth:** Uses the notebook identity via `notebookutils.credentials.getToken`.
...
# -------------------------------------------------------
CAPACITY_ID = ""  # e.g. "a1b2c3d4-0000-0000-0000-112233445566"
...
# -------------------------------------------------------
ENVIRONMENTS = ["Dev", "Test", "Prod"]  # options: "Dev", "Test", "Prod"
...
CREATE_PIPELINES         = True  # Deployment pipelines Dev→Test→Prod (requires >= 2 environments)
...
# -------------------------------------------------------
MULTIPLE_BC_ENVIRONMENTS = False  # True = Raw lakehouse created with schemas enabled
...
def _post(path: str, body: dict) -> dict:
    ...
    # 202 Accepted = long-running operation; return location for polling if needed
...
def assign_to_capacity(workspace_id: str) -> None:
    """Assigns a workspace to the configured capacity."""
    if not CAPACITY_ID:
...
def create_folder(workspace_id: str, folder_name: str) -> str:
    """Creates a folder in a workspace and returns its ID."""
...
def create_lakehouse(workspace_id: str, lakehouse_name: str, folder_id: str = None, enable_schemas: bool = False) -> str:
    """Creates a lakehouse, optionally inside a folder, and returns its ID."""
...
def create_mirrored_database(workspace_id: str, name: str) -> str:
    """
    Creates a GenericMirror (Open Mirroring) mirrored database.
    No source connection is required — the source can be configured afterwards.
    """
    mirroring_def = {
        "properties": {
            "source": {
    ...
    print(f"    ℹ  Open Mirroring — configure the source connection in the Fabric UI when ready.")
    return item_id
